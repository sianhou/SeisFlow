...
from models.wrapper import AutoencoderKLWrapper


def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Validate a Diffusers SeismicAutoencoderKL checkpoint on random "
            "SEG-Y shot crops."
        ),
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to a checkpoint directory produced by "
            "train_seismic_vae.py/save_training."
        ),
    )
    parser.add_argument(
        "--segy",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory used for validation outputs. "
            "Default: <ckpt_parent>/valid_<ckpt_name>."
        ),
    )
    parser.add_argument(
        "--batch_size",
    ...
    parser.add_argument(
        "--device",
        default="cuda",
        help="Validation device, such as cuda, cuda:0, or cpu.",
    )
    parser.add_argument(
        "--seed",
        default=0,
...
def load_model(checkpoint_dir, device):
    ...
    model = AutoencoderKLWrapper.from_pretrained(
        checkpoint_dir,
        device=device,
    )
    model.eval()
    return model, dict(model.model.config)


def get_input_size(config):
    sample_size = config.get("sample_size")
    if isinstance(sample_size, (list, tuple)):
        if len(sample_size) != 2 or sample_size[0] != sample_size[1]:
            raise ValueError(f"Expected square VAE sample_size, got {sample_size}.")
        sample_size = sample_size[0]
    if sample_size is None:
        raise ValueError("VAE checkpoint config does not define sample_size.")
    return int(sample_size)


def get_checkpoint_epoch(checkpoint_dir):
    prefix = "checkpoint_epoch_"
    name = Path(checkpoint_dir).name
    if not name.startswith(prefix):
        return None
    epoch_text = name[len(prefix):]
    return int(epoch_text) if epoch_text.isdigit() else None


def validate_model_io(config, input_tensor, outputs, input_size):
    expected_input_channels = int(config.get("in_channels", 1))
    expected_output_channels = int(config.get("out_channels", 1))
    if expected_output_channels != 1:
        raise ValueError(
            "SEG-Y validation currently requires a single reconstruction channel, "
            f"but the checkpoint defines {expected_output_channels}."
        )

    if input_tensor.ndim != 4:
        raise ValueError(
            f"Expected model input with shape [B,C,H,W], got {tuple(input_tensor.shape)}."
        )
    if input_tensor.shape[1] != expected_input_channels:
        raise ValueError(
            "Checkpoint/input channel mismatch: "
            f"model expects {expected_input_channels}, "
            f"validation data provides {input_tensor.shape[1]}."
        )
    if input_tensor.shape[-2:] != (input_size, input_size):
        raise ValueError(
            f"Expected {input_size}x{input_size} model input, "
            f"got {tuple(input_tensor.shape[-2:])}."
        )

    required_outputs = {"recon", "mean", "logvar"}
    missing_outputs = required_outputs.difference(outputs)
    if missing_outputs:
        raise KeyError(
            f"Model output is missing fields: {sorted(missing_outputs)}."
        )
    ...
    if reconstruction.shape != (
        ...
        raise ValueError(
            ...
            f"expected {(input_tensor.shape[0], expected_output_channels, input_size, input_size)}, "
            f"got {tuple(reconstruction.shape)}."
        )
    if outputs["mean"].shape != outputs["logvar"].shape:
        raise ValueError(
            "Posterior mean/logvar shape mismatch: "
            f"{tuple(outputs['mean'].shape)} vs {tuple(outputs['logvar'].shape)}."
        )
...
def validate(args):
    ...
    default_output_name = f"valid_{checkpoint_dir.name}"
    ...
    output_dir = (
        ...
        if args.output_dir
        else checkpoint_dir.parent / default_output_name
    )
    output_dir.mkdir(parents=True, exist_ok=True)

    model, config = load_model(checkpoint_dir, device)
    crop_size = get_input_size(config)
    ...
    with torch.inference_mode():
        outputs = model(input_tensor, sample_posterior=False)
        validate_model_io(config, input_tensor, outputs, crop_size)
        recon_normalized_batch = outputs["recon"].squeeze(1).detach().cpu().numpy()
        latent_mu_batch = outputs["mean"].detach().cpu().numpy()
        latent_logvar_batch = outputs["logvar"].detach().cpu().numpy()
        latent_z_batch = latent_mu_batch.copy()
    ...
    summary = {
        ...
        "checkpoint_epoch": get_checkpoint_epoch(checkpoint_dir),
        "model_type": "diffusers_autoencoder_kl",
        "segy": args.segy,
        "device": str(device),
        ...
        "latent_selection": "posterior_mode",
