...
class DiTSeisDimReconNeRFTrainer(DistributedTrainer):
    ...
    def validate_batch(self, clean_images, conditioning):
        if clean_images.shape[1] != 1:
            raise ValueError(
                "DiTSeisDimReconNeRF expects single-channel image patches, "
...
class DiTSeisDimReconNeRFInference(DistributedInference):
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Train or validate DiTSeisDimReconNeRF, a distributed conditional flow-matching model "
        ...
        epilog=(
            ...
            "  Valid:\n"
            "  torchrun --nproc_per_node=4 DiTSeisDimReconNeRF.py valid "
...
def run(args):
    if args.mode == "valid":
        return DiTSeisDimReconNeRFInference(args).run()
