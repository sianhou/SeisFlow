...
class LogReader:
    ...
    Example:
        reader = LogReader("output/run/train.log")
        losses = reader["loss"]
        first_row = reader[0]

    Read one channel from a SimpleLogger2 run directory:
        reader = LogReader("output/run", channel="train")
    ...
    Generate text files directly from the command line:
        python core/logging/logreader.py --path output/run --channel train
        python core/logging/logreader.py --path output/run/train.log
    """
    ...
    def _parse_lines(self, lines: Iterable[str]):
        ...
        for line_number, raw_line in enumerate(lines, start=1):
            ...
            if not header:
                if self.strict:
                    raise ValueError(f"[L] row before [H] header at line {line_number}.")
                continue
    ...
    def _resolve_path(path: PathLike, channel: Optional[str]) -> Path:
        ...
        if log_path.is_dir():
            if channel is None:
                channel = "train"
            log_path = log_path / f"{channel}.log"
        return log_path
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--path",
        required=True,
        help="Run directory or one .log file generated by SimpleLogger2.",
    )
    parser.add_argument(
        "--channel",
        default="train",
        help="Channel name when path is a run directory. Default: train.",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
        help="Directory for exported column files. Default: <log_stem>_columns beside the log.",
    )
    parser.add_argument(
        "--value_width",
        default=None,
