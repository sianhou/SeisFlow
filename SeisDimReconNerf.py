...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Train a conditional flow-matching model from noise to seismic patches with NeRF-encoded dimension coordinates."
        ),
        epilog=(
            "Examples:\n"
            "  Train:\n"
            "  python SeisDimReconNerf.py "
            "--train_data_dir ./dataset256/train "
            "--train_data_dim_dir ./dataset256/train_dim "
            "--output_dir ./output_dim_recon "
            "--model_arch DiT_T_4 --input_size 256 --batch_size 32 --num_epochs 1000 "
            "--device cuda\n\n"
            "  Valid:\n"
            "  python SeisDimReconNerf.py valid "
...
def validate_nerf_dim_model_channels(model, raw_dim_channels, args):
    nerf_dim_channels = get_nerf_conditioning_channels(raw_dim_channels, args)
    validate_dim_model_channels(model, nerf_dim_channels)
...
def reconstruct_dim_file(
    ...
    for batch_start in range(0, num_patches, args.batch_size):
        ...
        conditioning = encode_nerf_conditioning(conditioning, args)
        noise = torch.randn(
            (
...
def run_train(args):
    ...
    nerf_dim_channels = get_nerf_conditioning_channels(dim_channels, args)
    logger.log_event(
        "dataset_initialized",
...
def run_valid(args):
    ...
    nerf_dim_channels = validate_nerf_dim_model_channels(model, dim_channels, args)
