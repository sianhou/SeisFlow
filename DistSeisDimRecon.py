...
class SeisDimReconTrainer(DistributedTrainer):
    ...
    def validate_batch(self, clean_images, conditioning):
        if clean_images.shape[1] != 1:
            raise ValueError(
                "SeisDimRecon expects single-channel image patches, "
...
class SeisDimReconInference(DistributedInference):
...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  Train:\n"
            "  torchrun --nproc_per_node=4 DistSeisDimRecon.py "
            ...
            "  Valid:\n"
            "  torchrun --nproc_per_node=4 DistSeisDimRecon.py valid "
...
def run_train(args):
    args.mode = "train"
    trainer = SeisDimReconTrainer(args)
    return trainer.run()


def run_valid(args):
    args.mode = "valid"
    inference = SeisDimReconInference(args)
