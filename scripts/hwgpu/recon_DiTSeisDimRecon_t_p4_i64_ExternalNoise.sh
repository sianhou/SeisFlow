...
TRAIN_ROOT="${TRAIN_ROOT:-$PROJ_DIR/train_DistSeisDimRecon_t_p4_i64}"
...
[[ -d "$NOISE_DATA_DIR/valid" ]] || {
    echo "External noise patches not found: $NOISE_DATA_DIR/valid" >&2
    exit 1
}
[[ -d "$TRAIN_RUN_DIR" ]] || {
...
for epoch in $(seq 100 100 1000); do
    ...
    echo "Reconstructing epoch $epoch with external noise from $checkpoint_dir"
    "$TORCHRUN_BIN" \
        --nnodes=1 \
        ...
        DiTSeisDimReconExternalNoise.py valid \
        --ckpt "$checkpoint_dir" \
