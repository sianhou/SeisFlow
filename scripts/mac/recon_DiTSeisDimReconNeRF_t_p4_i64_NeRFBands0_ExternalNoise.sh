...
NOISE_DATA_DIR="${NOISE_DATA_DIR:-$PROJ_DIR/random_shot_dataset64}"
RUN_DIR="${RUN_DIR:-$EXPERIMENT_DIR/$SCRIPT_NAME}"
BASE_SCRIPT_NAME="${SCRIPT_NAME%_ExternalNoise}"
TRAIN_SCRIPT_NAME="${BASE_SCRIPT_NAME/#recon_/train_}"
...
[[ -d "$NOISE_DATA_DIR/valid" ]] || { echo "External noise patches not found: $NOISE_DATA_DIR/valid" >&2; exit 1; }
...
[[ -f "$NOISE_DATA_DIR/valid/patches_${SHOT_ID}.npy" ]] || { echo "Shot noise patches not found: patches_${SHOT_ID}.npy" >&2; exit 1; }
...
SUBSET_NOISE_DIR="$SUBSET_DIR/noise"
mkdir -p "$SUBSET_DIM_DIR" "$SUBSET_AUX_DIR" "$SUBSET_NOISE_DIR"
ln -s "$DATA_DIR/valid_dim/patches_${SHOT_ID}.npy" "$SUBSET_DIM_DIR/"
ln -s "$DATA_DIR/valid_aux/patches_${SHOT_ID}.npz" "$SUBSET_AUX_DIR/"
ln -s "$NOISE_DATA_DIR/valid/patches_${SHOT_ID}.npy" "$SUBSET_NOISE_DIR/"
...
echo "NOISE_DATA_DIR: $SUBSET_NOISE_DIR"
echo "DEVICE: $DEVICE"
...
for weight_set in ema no_ema; do
    ...
    log_id="valid_${weight_set}_shot_${SHOT_ID}_epoch_${epoch_name}"
    patch_output_dir="$RUN_DIR/$log_id"
    shot_output_dir="$RUN_DIR/valid_recon_shot_${weight_set}_shot_${SHOT_ID}_epoch_${epoch_name}"
    diff_output_dir="$RUN_DIR/diff_recon_shot_${weight_set}_shot_${SHOT_ID}_epoch_${epoch_name}"

    echo "Reconstructing epoch $EPOCH with $weight_set weights and external noise"
    "$PYTHON_BIN" "$CODE_PATH/DiTSeisDimReconNeRFExternalNoise.py" sample \
        --ckpt "$checkpoint_dir" \
        --input_dim_dir "$SUBSET_DIM_DIR" \
        --noise_data_dir "$SUBSET_NOISE_DIR" \
        --output_dir "$RUN_DIR" \
        --log_id "$log_id" \
        --model_arch DiT_T_4 \
        --input_size 64 \
    ...
    echo "Reconstructing shots for epoch $EPOCH with $weight_set weights"
    ...
    echo "Generating shot differences for epoch $EPOCH with $weight_set weights"
