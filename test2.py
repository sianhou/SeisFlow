...
def train():
    ...
    for epoch in range(1000):
        ...
        for data_iter_step, (samples) in enumerate(dataloader):
            ...
            missed = mask * samples
            condition = {}
            condition["concat_conditioning"] = torch.cat((missed, mask), dim=1)
            ...
            # ===== 前向 =====
            pred = model(x_t, t, extra=condition)
        ...
        # ===== 每100个epoch保存模型 =====
        if (epoch + 1) % 100 == 0:
            save_path = f"test2_model_epoch_{epoch + 1:04d}.pth"
...
def eval(checkpoint):
    ...
    synthetic_samples = solver.sample(
        ...
        cfg_scale=0.0,
        label=None,
        concat_conditioning=condition
    )

    # plot
    imgs = mask.detach().cpu().numpy()
    plot4(imgs, "mask.png")

    imgs = missed.detach().cpu().numpy()
    plot4(imgs, "missed.png")

    imgs = synthetic_samples[:, 0]  # [B,H,W]
    imgs = imgs.detach().cpu().numpy()
    plot4(imgs, "recon.png")

    diff = samples - synthetic_samples
    imgs = diff.detach().cpu().numpy()
    plot4(imgs, "diff.png")


if __name__ == '__main__':
    # train()
    eval(f"test2_model_epoch_0100.pth")
    # for epoch in range(1000):
    #     if (epoch + 1) % 100 == 0:
