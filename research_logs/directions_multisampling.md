...
- 当前判断：多次独立采样平均是有效的推理端优化，能够降低随机方差、MSE、MAE 和极端误差；但它不是当前关键问题，且收益递减、计算成本近似线性增长，也不能消除 patch 上下文不足造成的结构性误差，因此暂时保留而不作为下一步重点。
...
1. 暂不将多次独立采样作为下一步重点，优先解决 patch 上下文不足等结构性问题。
2. 后续需要推理端优化时，在当前最佳 bands3 + EMA checkpoint 上比较 1、2、4 次独立采样。
3. 先测试最差 16 炮和若干简单炮；如果大部分炮稳定提高，再扩展到完整 75 炮验证集。
