...
def build_parser():
    ...
    parser.add_argument(
        "--ckpt",
        required=True,
        help="Path to a VAE .pth file saved with save_checkpoint.",
    )
    parser.add_argument(
        "--segy",
        required=True,
...
def load_model(checkpoint_path, device):
    ...
    config = checkpoint_metadata.get("model_config")
    if not config:
        raise ValueError(
            f"Checkpoint does not contain model_config: {checkpoint_path}"
        )

    model = AutoencoderKLWrapper(AutoencoderKL.from_config(config)).to(device)
    _, checkpoint = model.load_checkpoint(checkpoint_path, device=device)
    model.eval()
    return model, checkpoint, dict(model.model.config)


def get_input_size(config, checkpoint):
    sample_size = config.get("sample_size")
    if sample_size is None:
        sample_size = checkpoint.get("args", {}).get("input_size")
    ...
    if sample_size is None:
        raise ValueError("VAE checkpoint does not define sample_size or input_size.")
...
def validate(args):
    ...
    crop_size = get_input_size(config, checkpoint)
